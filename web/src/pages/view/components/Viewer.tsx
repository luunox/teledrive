...
import { Button, Descriptions, Divider, Dropdown, Input, Layout, Menu, message, notification, Space, Tag, Typography } from 'antd';
...
const Viewer: React.FC<Props> = ({ data, me, error, mutate, pageParams, isInDrawer, onCloseDrawer }) => {
  ...
  const { data: datafilesParts } = useSWR(
    data?.file.name && /\.part0*\d+$/.test(data.file.name)
      ? `/files?name.match=${encodeURIComponent('.part0*[0-9]+$')}&name.like=${data.file.name.replace(/\.part0*\d+$/, '')}%&user_id=${data.file.user_id}${
          me?.user.id !== data.file.user_id ? '&shared=1' : ''
        }&parent_id${data.file.parent_id ? `=${data.file.parent_id}` : '.is=null'}`
      : null,
    fetcher
  );
  ...
  return (
    <>
      <Layout style={{ height: '100vh', overflow: 'hidden', background: '#2a2a2a', color: 'rgb(251,251,254)' }}>
        <Layout.Content>
        ...
        <Layout.Sider width={320} trigger={null} collapsedWidth={0} breakpoint='lg' collapsed={collapsed} onCollapse={setCollapsed}>
          <Layout.Content className='container' style={{ ...(contentStyle || {}), color: '#fff', margin: '70px 10px' }}>
            <Descriptions
              title={
                <Typography.Text style={{ color: '#fff' }}>
                  <Icon type={data?.file.type} /> &nbsp; {data?.file.name.replace(/\.part0*\d+$/, '')}
                </Typography.Text>
              }
              ...
              <Descriptions.Item label='Size'>
                {datafilesParts?.length
                  ? prettyBytes(datafilesParts?.files.reduce((res: number, file: any) => res + Number(file.size), 0)) + ` (${datafilesParts?.length} parts)`
                  : data?.file.size && prettyBytes(Number(data?.file.size || 0))}
              </Descriptions.Item>
              ...
              {user?.user && (
                <Descriptions.Item label='Uploaded By'>
                  <a href={`https://t.me/${user?.user.username}`} target='_blank'>
                    @{user?.user.username}
                  </a>
                </Descriptions.Item>
              )}
            ...
            <Divider />
            <Descriptions colon={false} layout='vertical' labelStyle={{ color: '#fff' }} column={1}>
              <Descriptions.Item
                label={
                  <>
                    <LinkOutlined /> &nbsp; Raw URL
                  </>
                }>
        ...
        <div style={{ position: 'absolute', right: 20, top: 30 }}>
          <Space direction='horizontal'>
            ...
            {!showContent && me?.user.id === data?.file.user_id ? (
              <Dropdown
                placement='bottomCenter'
                trigger={['click']}
                overlay={
                  <Menu>
                    <Menu.Item key='rename' onClick={() => setFileRename(data?.file)} icon={<EditOutlined />}>
                      Rename
                    </Menu.Item>
                    <Menu.Item key='share' onClick={() => setSelectShare({ action: 'share', row: data?.file })} icon={<ShareAltOutlined />}>
                      Share
                    </Menu.Item>
                    <Menu.Item key='send' onClick={() => setSelectShare({ action: 'forward', row: data?.file })} icon={<ArrowRightOutlined />}>
                      Send to
                    </Menu.Item>
                    <Menu.Item key='download' onClick={() => window.open(links?.download)} icon={<DownloadOutlined />}>
                      Download
                    </Menu.Item>
                    <Menu.Item key='fastdownload' onClick={() => directDownload(data?.file.id, data?.file.name.replace(/\.part0*\d+$/, ''))} icon={<CloudDownloadOutlined />}>
                      Fast Download <Tag color='green'>beta</Tag>
                    </Menu.Item>
                    <Menu.Item key='remove' danger onClick={() => setSelectDeleted([data?.file])} icon={<DeleteOutlined />}>
                      Delete
                    </Menu.Item>
                  </Menu>
                }>
              ...
              <Button shape='circle' onClick={() => window.open(links?.download)} icon={<DownloadOutlined />} />
            )}
      ...
      <Rename dataSelect={[fileRename, setFileRename]} onFinish={mutate} />
      ...
      <Share me={me} dataSelect={[selectShare, setSelectShare]} onFinish={mutate} />
    </>
  );
};
